...
export const ServerSearch = ({
    ...
    const onClick = ({ id, type }: { id: string, type: "channel" | "member" }) => {
        setOpen(false);
        if( type === "member" ) {
            return router.push(`/servers/${params?.serverId}/consversation/${id}`)
        }
        if( type === "channel" ) {
    ...
    return <>
        <button 
            ...
            className="group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-xinc-700/10 dark:hover:bg-zinc-700/50 transition"
        >
